...
struct EnvelopeQ15 {
    ...
    int32_t __not_in_flash_func(Process)(uint8_t flags) {
        ...
        if (!sustained && !done) {
            ...
            // Scale from Q20 to Q15: shift right by 5
            // But we need the increment to drive phase (Q15), so:
            // phase += increment_q20 >> 5 would lose precision.
            // Instead, keep phase in Q20 internally? No, keep it simple:
            // The original uses phase 0..1 with float increments ~0.0001..0.05
            // In Q15: increment range = 3..1638 — fine for integer accumulation.
            phase_increment >>= 5;
        }
