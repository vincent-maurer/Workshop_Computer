...
export class PicobootCmd {
    ...
    /**
     * Run Thumb code already placed in RAM, without USB re-enumeration.
     * The bootrom calls the function on its own stack and resumes after it
     * returns, so the PICOBOOT session stays alive.
     * @param {number} addr - entry point; bit 0 must be set (Thumb)
     * @returns {PicobootCmd}
     */
    static exec(addr) {
        const args = new Uint8Array(16);
        new DataView(args.buffer).setUint32(0, addr, true);
        return new PicobootCmd(PicobootCmdId.EXEC, 4, 0, args);
    }
