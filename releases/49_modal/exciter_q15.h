...
struct ExciterQ15 {
    ...
    int32_t __not_in_flash_func(ProcessMallet)(uint8_t flags) {
        ...
        if (!(flags & ENV_FLAG_GATE)) {
            // Exponential decay of damping: 1 - 0.95*(1-damp)
            // In Q15: damp = 32767 - ((32767 - damp) * 31129) >> 15
            //   where 31129 ≈ 0.95 * 32767
    ...
    int32_t __not_in_flash_func(ProcessPlectrum)(uint8_t flags) {
        ...
        if (flags & ENV_FLAG_RISING) {
            ...
            // Delay before snap: 64 + 4096 * parameter²
            int32_t p2 = mul_q15(parameter, parameter);
            plectrum_delay = 64 + (uint32_t)((p2 * 4096) >> 15);
        }
        ...
        if (plectrum_delay > 0) {
            ...
            // Damping builds during pre-snap: 1 - 0.997*(1-damp)
            damp_state = 32767 - (int32_t)(((int64_t)(32767 - damp_state) * 32669) >> 15);
        } else {
            // After snap: damping decays: damp *= 0.9
            damp_state = (int32_t)(((int64_t)damp_state * 29491) >> 15);
        }
